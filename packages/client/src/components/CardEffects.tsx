...
export function EffectView({ effect }: { effect: CardEffect }) {
  switch (effect.kind) {
    case "resource":
      return (
        <>
          Produce {effect.production.qty > 1 ? `${effect.production.qty}x ` : ""}
    ...
    case "shields":
      return (
        <>
          +{effect.count} <ShieldIcon /> shield{effect.count > 1 ? "s" : ""}
        </>
      );
    case "science":
      return (
    ...
    case "vpAndCoinsPerCard":
      return (
        <>
          +{effect.coinsPer} 🪙 now, +{effect.vpPer} 🏆 per <ColorTag color={effect.color} /> card in {SCOPE_LABEL[effect.scope]}
        </>
      );
    ...
    case "vpPerColorSet":
      return (
        <>
          +{effect.perCard} 🏆 per {joinSlash(effect.colors.map((c) => <ColorTag key={c} color={c} />))} card in your city
        </>
      );
    case "tradeDiscount":
      return (
        <>
          Trade {effect.resources.map((r) => <ResourceIcon key={r} type={r} />)} from {effect.sides.join("/")} neighbor for {effect.unitCost} 🪙
        </>
      );
    case "copyGuild":
      return (
    ...
    case "opponentsPayPerOwnMetric":
      return (
        <>
          Every other player pays {effect.perUnit} 🪙 per {effect.metric === "wonderStagesBuilt" ? "Wonder stage" : "military victory"} they have
        </>
      );
    ...
    case "buildDiscount":
      return (
        <>
          Build {effect.appliesTo === "wonderStage" ? "Wonder stages" : <ColorTag color={effect.appliesTo} />} for {effect.units} fewer resource{effect.units > 1 ? "s" : ""}
        </>
      );
    ...
    case "bankPurchase":
      return (
        <>
          Once per turn, buy 1 resource from the bank for {effect.unitCost} 🪙
        </>
      );
    case "neighborPurchaseRebate":
      return (
        <>
          Refund {effect.amount} 🪙 per neighbor you trade with this turn
        </>
      );
    case "vpPerColorSetBonus":
      return (
        <>
          +{effect.perSet} 🏆 per complete set of {joinSlash(effect.colors.map((c) => <ColorTag key={c} color={c} />))} in your city
        </>
      );
    ...
    case "vpPerCoinsHeld":
      return (
        <>
          +1 🏆 per {effect.coinsPerVp} 🪙 held at game end
        </>
      );
...
export function CardEffectsView({ card }: { card: Pick<Card, "effects"> }) {
  return (
    <>
